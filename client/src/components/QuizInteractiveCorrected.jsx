...
const QuizInteractiveCorrected = () => {
            ...
            {selected.courses.map((course, idx) => (
              <div key={course._id || idx} className="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-5 border-t-4" style={{ borderColor: selected.color || '#a78bfa' }}>
                ...
                {course.questions && Array.isArray(course.questions) && (
                  <QuizBlock questions={course.questions} corrections={course.corrections} />
                )}
              </div>
            ))}
          </div>
...
function QuizBlock({ questions, corrections }) {
  const [answers, setAnswers] = useState(Array(questions.length).fill(''));
  const [show, setShow] = useState(false);
  const [score, setScore] = useState(null);

  const handleCorrection = () => {
    if (!corrections || !Array.isArray(corrections)) return null;
    let sc = 0;
    answers.forEach((ans, i) => {
      if (typeof corrections[i] === 'string' && ans.trim().toLowerCase() === corrections[i].trim().toLowerCase()) {
        sc++;
      }
    });
    setScore(sc);
  ...
  return (
    <div className="mt-4">
      ...
      >
        {questions.map((q, i) => (
          <div key={i} className="flex flex-col gap-1">
            ...
            <input
              ...
              disabled={show}
            />
            {show && corrections && corrections[i] && (
              <div className={answers[i].trim().toLowerCase() === corrections[i].trim().toLowerCase() ? 'text-green-600' : 'text-red-600'}>
        ...
        {show && corrections && (
          <div className="mt-2 text-green-700 font-bold">Score : {score} / {questions.length}</div>
        )}
        {show && !corrections && (
          <div className="mt-2 text-green-700 font-bold">Réponses enregistrées ! (correction non disponible)</div>
        )}
      </form>
    </div>
  );
...
export default QuizInteractiveCorrected;
